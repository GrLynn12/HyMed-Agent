...
You can use our fine-tuned model in `experiments/finetune/inference_hf.py`, which can be easily tested with just one line of
code.
...
1. Use the method of reading the model in `experiments/finetune/inference_hf.py` to replace the method of reading the model in
   the demo.
